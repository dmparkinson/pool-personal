...
if (!function_exists('niso_slider_carousel_group_tab')) :
	...
	function niso_slider_carousel_group_tab()
	{
		...
		$niso_meta->add_field(array(
			...
			'desc'       => __(' 
        <div class="niso-prolink"> 
            <a class="niso-button" href="https://plugin.wpteamx.com/niso-doc/" target="_blank">Documentation</a>
        </div>
...
if (!function_exists('wpspace_notice__error')) {
	...
	function wpspace_notice__error()
	{
		...
		$url1 = esc_url('https://wpthemespace.com/product/niso-carousel/?add-to-cart=271');
		$url2 = esc_url('https://plugin.wpteamx.com/');

		printf('<div class="%1$s" style="padding:10px 15px 20px;text-transform:uppercase"><p>%2$s</p><a target="_blank" class="button button-danger" href="%3$s" style="margin-right:10px">' . __('Upgrade Pro', 'niso') . '</a><a target="_blank" class="button button-primary" href="%4$s">' . __('Show Demo', 'niso-carousel') . '</a><a style="margin-left:10px;cursor: pointer;" class="niso-dismiss">Dissimiss the notice</a></div>', esc_attr($class), wp_kses_post($message), $url1, $url2);
	}
